...
def play action
  ...
  when 1
      ...
      until @toyrobot.instance_variable_get("@facing").include? facing
        puts "\e[#{31}m#{"Wrong spelling, please enter again"}\e[0m"
        facing = gets.chomp.downcase
      end
  ...
  when 2
    if @toyrobot.on_tabletop? @tabletop.tabletop
      ...
      puts "\e[#{31}m#{"Can't move, robot not on tabletop"}\e[0m"
    end
    actions
  when 3
    if @toyrobot.on_tabletop? @tabletop.tabletop
      ...
      puts "\e[#{31}m#{"Can't rotate, robot is not on tabletop"}\e[0m"
    end
    actions
  when 4
    if @toyrobot.on_tabletop? @tabletop.tabletop
  ...
  when 5
    if @toyrobot.on_tabletop? @tabletop.tabletop
      ...
      Y axia - #{@toyrobot.coordinates[1]}
      Facing - #{@toyrobot.coordinates[2]}", ""
    else
      puts "\e[#{31}m#{"Can't report position, robot not on tabletop"}\e[0m"
    end
    actions
  when 6
    exit
